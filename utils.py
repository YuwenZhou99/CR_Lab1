...
class IsolatedObjData:
    ...
    def __init__(self, obj_names, trials, save_path):
        # List of object names and number of trials
        self.obj_names = obj_names
        self.trtrials = trials  # NOTE: kept original variable name for compatibility if used elsewhere
        self.trials = trials
...
def plot(path, tries, target, grasp, trials):
    ...
    for obj in succes_rate.keys():
        t = tries[obj]
        # If there were no trials for this object, avoid division by zero by
        # treating the denominator as 1 (this keeps the plot code simple)
        if t == 0:
            t = 1
